...
// Whalecome to SmoothSyncMLAPI. If you have any problems, suggestions, or comments, don't hesitate to let us hear them.
// With Love,
// Noble Whale Studios
...
namespace Smooth
{
    ...
    /// </remarks>
    public class SmoothSyncMLAPI: NetworkBehaviour
    {
        ...
        /// In order to sync a child object, you must add two instances of SmoothSyncMLAPI to the parent. 
        ...
        /// To tie in your own validation method, check the SmoothSyncMLAPIExample scene and 
        /// SmoothSyncMLAPIExamplePlayerController.cs on how to use the validation delegate.
        ...
        /// </remarks>
        public delegate bool validateStateDelegate(StateMLAPI receivedState, StateMLAPI latestVerifiedState);
        ...
        /// </remarks>
        public static bool validateState(StateMLAPI latestReceivedState, StateMLAPI latestValidatedState)
        {
            return true;
        }
        ...
        /// SmoothSyncMLAPI.validateStateMethod = myCoolCustomValidatePlayerMethod; 
        ...
        /// </remarks>
        StateMLAPI latestValidatedState;
        ...
        [NonSerialized]
        public SmoothSyncMLAPI[] childObjectSmoothSyncs = new SmoothSyncMLAPI[0];
        ...
        public void Awake()
        {
            ...
            stateBuffer = new StateMLAPI[Mathf.Max(calculatedStateBufferSize, 30)];
            ...
            targetTempState = new StateMLAPI();
            sendingTempState = new StateMLAPI();
        }
        ...
        public void SetObjectToSync(GameObject childObjectToSync)
        {
            ...
            if (childObjectToSync)
            {
                ...
                // Throw error if no SmoothSyncMLAPI script is handling the parent object.
                bool foundAParent = false;
                childObjectSmoothSyncs = GetComponents<SmoothSyncMLAPI>();
                ...
                if (!foundAParent)
                {
                    Debug.LogError("You must have one SmoothSyncMLAPI script with unassigned childObjectToSync in order to sync the parent object");
                }
            }
        ...
        public void OnEnable()
        {
            ...
            if (!NetworkManager.Singleton.IsServer) registerClientHandlers();
        }
        ...
        void sendState()
        {
            ...
            else if (NetworkManager.Singleton.IsClient)
            {
                ...
                using (var writer = PooledNetworkWriter.Get(stream))
                {
                    ...
                    CustomMessagingManager.SendNamedMessage("HandleSync", NetworkManager.Singleton.ServerClientId, stream);
                }
            }
        }
        ...
        void interpolate(float interpolationTime)
        {
            ...
            {
                //Debug.LogError("Ran out of States in SmoothSyncMLAPI's State buffer for object: " + gameObject.name);
                stateIndex--;
            }

            // The State one slot newer than the starting State.
            StateMLAPI end = stateBuffer[Mathf.Max(stateIndex - 1, 0)];
            // The starting playback State.
            StateMLAPI start = stateBuffer[stateIndex];
            ...
            targetTempState = StateMLAPI.Lerp(targetTempState, start, end, t);
        ...
        void shouldTeleport(StateMLAPI start, ref StateMLAPI end, float interpolationTime, ref float t)
        {
        ...
        public void addState(StateMLAPI state)
        {
            if (stateCount > 1)
            {
        ...
        /// Full example of use in the example scene in SmoothSyncMLAPIExamplePlayerController.cs.
        /// </remarks>
        ...
        public void TeleportServerRpc(Vector3 position, Vector3 rotation, Vector3 scale, float tempOwnerTime)
        {
            ...
            StateMLAPI teleportState = new StateMLAPI();
        ...
        /// </summary>
        void addTeleportState(StateMLAPI teleportState)
        {
        ...
        public override void NetworkStart()
        {
            ...
            if (NetworkManager.Singleton.IsServer)
            {
                MLAPI.Messaging.CustomMessagingManager.RegisterNamedMessageHandler("HandleSync", HandleSync);
            }
            if (NetworkManager.Singleton.IsServer)
            {
        ...
        void SendStateToNonOwners(StateMLAPI state)
        {
            ...
            while (observers.MoveNext())
            {
                ...
                if ((transformSource == TransformSource.Server || conn != netIdentity.OwnerClientId) && conn != NetworkManager.Singleton.LocalClientId)
                {
                    ...
                    using (var writer = PooledNetworkWriter.Get(stream))
                    {
                        ...
                        CustomMessagingManager.SendNamedMessage("HandleSync", conn, stream);
                    }
                }
            }
        }

        static void HandleSync(ulong conn, Stream payload)
        {
            StateMLAPI networkState = null;
            using (var reader = PooledNetworkReader.Get(payload))
            {
                networkState = StateMLAPI.Deserialize(reader);
            }
            if (NetworkManager.Singleton.IsServer)
            {
                ...
                // To tie in your own validation method, see the SmoothSyncMLAPIExample scene and SmoothSyncMLAPIExamplePlayerController.cs. 
        ...
        public void checkIfOwnerHasChanged(StateMLAPI newState)
        {
                ...
                ownerChangeIndicator != previousReceivedOwnerInt)
            {
                ...
                StateMLAPI simulatedState = new StateMLAPI();
