...
/// </remarks>
public class SmoothSyncMLAPIExamplePlayerController : NetworkBehaviour
{
    Rigidbody rb;
    Rigidbody2D rb2D;
    SmoothSyncMLAPI smoothSync;
	...
	void Start ()
    {
        ...
        smoothSync = GetComponent<SmoothSyncMLAPI>();
	...
	void Update ()
    {
        ...
        if (!(GetComponent<NetworkObject>().IsOwner || (NetworkManager.Singleton.IsServer && GetComponent<NetworkObject>().IsOwnedByServer))) return;
    ...
    /// </remarks>
    /// </summary>
    public static bool validateStateOfPlayer(StateMLAPI latestReceivedState, StateMLAPI latestValidatedState)
    {
